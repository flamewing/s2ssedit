...
void sseditor::on_pastebutton_clicked() {
    ...
    for (auto const& elem : copystack) {
        int newpos = delta + static_cast<int>(
                                 segpos[elem.get_segment()] + elem.get_pos());
        if (newpos < 0) {
            newpos = 0;
        ...
        int newseg = static_cast<int>(get_segment(newpos));
...
void sseditor::on_insert_segment_before_button_clicked() {
    auto act = make_shared<insert_segment_action>(
        currstage, currsegment, sssegments());
    do_action(act);
...
void sseditor::on_cut_segment_button_clicked() {
    ...
    auto act =
        make_shared<cut_segment_action>(currstage, currsegment, *copyseg);
    do_action(act);
...
void sseditor::on_paste_segment_button_clicked() {
    auto act =
        make_shared<paste_segment_action>(currstage, currsegment, *copyseg);
    do_action(act);
...
bool sseditor::move_object(int dx, int dy) {
    ...
    for (auto const& elem : selection) {
        ...
        auto oldx   = static_cast<int>(elem.get_angle());
        auto oldy   = static_cast<int>(elem.get_pos());
        ...
        constexpr const int max_angle = 0x40;

        int newx = static_cast<uint8_t>(oldx + dx);
        int newy = (oldy + dy) % max_angle;
        int cnt  = max_angle;

        while (cnt > 0 && currseg->exists(newy, newx, type) &&
               selection.find(object(oldseg, newx, newy, type)) ==
                   selection.end()) {
            newx = static_cast<uint8_t>(newx + dx);
            newy += dy;
            newy %= max_angle;
            cnt--;
        }
...
void sseditor::show() {
    ...
    for (int i = start; i <= end && !hotspot.valid(); i++) {
        ...
        for (auto const& elem : row) {
            int ty = (i - pvscrollbar->get_value()) * IMAGE_SIZE,
                tx = angle_to_x(elem.first) - HALF_IMAGE_SIZE;
...
bool sseditor::on_specialstageobjs_button_press_event(GdkEventButton* event) {
    ...
    int pos = event->y / IMAGE_SIZE + pvscrollbar->get_value(),
        seg = get_segment(pos);
    lastclick.set(
        seg, x_to_angle_constrained(event->x), pos - segpos[seg],
        sssegments::eRing);
...
bool sseditor::on_specialstageobjs_button_release_event(GdkEventButton* event) {
    ...
    if (!hotspot.valid()) {
        if (snaptogrid ^ ((state & GDK_CONTROL_MASK) != 0)) {
            angle = x_to_angle_constrained(event->x + QUARTER_IMAGE_SIZE);
        } else {
        ...
        if (event->button == 1) {
            ...
            if (currseg->exists(pos, angle, type)) {
                if (type == sssegments::eBomb) {
                    ...
                    act = make_shared<alter_selection_action>(
    ...
    case eDeleteMode: {
        ...
        if (!hotstack.empty()) {
            auto act =
                make_shared<delete_selection_action>(currstage, hotstack);
            do_action(act);
            hotstack.clear();
        } else {
            sssegments::ObjectTypes type;
            if (currseg->exists(pos, angle, type)) {
                ...
                temp.emplace(seg, angle, pos, type);
                auto act =
                    make_shared<delete_selection_action>(currstage, temp);
                do_action(act);
            }
        }
        break;
        ...
        if (event->button == 1) {
            if (!insertstack.empty()) {
                ...
                for (auto const& elem : insertstack) {
                    ...
                    sssegments::ObjectTypes type;
                    if (currseg->exists(
                            elem.get_pos(), elem.get_angle(), type)) {
                        delstack.emplace(
                            seg, elem.get_angle(), elem.get_pos(), type);
                    }
                }
                auto act = make_shared<insert_objects_ex_action>(
                    currstage, delstack, insertstack);
                do_action(act);
                insertstack.clear();
            }
        } else if (event->button == 3) {
    ...
    while (i < last) {
        ...
        if (fill) {
            ...
            for (int j = 4 * delta * cnt; j >= min; j -= dj) {
                ...
                tx          = angle_normal(static_cast<uint8_t>(angle - jang));
                insertstack.emplace(seg, tx, pos, type);
                tx = angle_normal(static_cast<uint8_t>(angle + jang));
                insertstack.emplace(seg, tx, pos, type);
            }
        } else {
            tx = angle_normal(static_cast<uint8_t>(angle - delta));
            insertstack.emplace(seg, tx, pos, type);
            tx = angle_normal(static_cast<uint8_t>(angle + delta));
            insertstack.emplace(seg, tx, pos, type);
        }
    ...
    if (drawbox || mode == eInsertRingMode || mode == eInsertBombMode ||
        ...
        if (!hotspot.valid()) {
            if (snaptogrid ^ ((event->state & GDK_CONTROL_MASK) != 0)) {
                angle1 = x_to_angle_constrained(event->x + QUARTER_IMAGE_SIZE);
            } else {
        ...
        int angle0 = lastclick.get_angle(),
        ...
        case eDeleteMode:
            if (dangle || dpos) {
                ...
                boxcorner.set(
                    seg1, static_cast<uint8_t>(angle1 + 0xc0),
                ...
                for (int i = pos0; i <= pos1; i++) {
                    ...
                    for (int j = angle0; j <= angle1; j++) {
                        int         angle   = static_cast<uint8_t>(j + 0xc0);
                        sssegments* currseg = currlvl->get_segment(seg);
                        sssegments::ObjectTypes type;
                        if (currseg->exists(pos, angle, type)) {
        ...
        case eInsertBombMode: {
            ...
            case eLine: {
                ...
                do {
                    ...
                    insertstack.emplace(seg, angle_normal(angle), pos, type);
                    angle = static_cast<uint8_t>(angle + angledelta);
                } while (i != pos1 + delta);
                break;
            }
            case eLoop: {
                ...
                for (int i = 0; i <= nobj; i++, pos0 += dy) {
                    ...
                    insertstack.emplace(
                        seg, angle_normal(static_cast<uint8_t>(angle)), pos,
                        type);
                    angle += delta;
                }
                break;
            ...
            case eDiamond: {
                ...
                while (i != pos1) {
                    ...
                    int tx =
                        angle_normal(static_cast<uint8_t>(angle - angledelta));
                    insertstack.emplace(seg, tx, pos, type);
                    tx = angle_normal(static_cast<uint8_t>(angle + angledelta));
                    insertstack.emplace(seg, tx, pos, type);
                }
                break;
            }
...
bool sseditor::on_drag_motion(
    ...
    int dpos;
    if (hotspot.valid()) {
        dangle = static_cast<int>(hotspot.get_angle());
        dpos =
            static_cast<int>(segpos[hotspot.get_segment()] + hotspot.get_pos());
    } else {
        if (snaptogrid) {
    ...
    dangle = static_cast<uint8_t>(dangle - lastclick.get_angle());
    dpos -=
        static_cast<int>(segpos[lastclick.get_segment()] + lastclick.get_pos());
    ...
    if (dangle == 0 && dpos == 0) {
        ...
        for (auto obj : sourcestack) {
            obj.set_angle(static_cast<uint8_t>(obj.get_angle() + dangle));
            int pos =
                static_cast<int>(segpos[obj.get_segment()] + obj.get_pos()) +
                dpos;
...
void sseditor::on_specialstageobjs_drag_data_received(
    ...
    if (selection_data.get_data_type() == "SpecialStageObjects" &&
        ...
        while (len > 0) {
            ...
            auto newtype  = sssegments::ObjectTypes(Read1(ptr));
            len -= 4;
        ...
        if (!equal(
                ...
                ObjectMatchFunctor())) {
            auto act = make_shared<move_objects_action>(
                currstage, sourcestack, selection);
            do_action(act);
        }
        sourcestack.clear();
    } else {
